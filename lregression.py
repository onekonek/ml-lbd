...
class Regression(object):
    ...
    def train(self, training_set):
        k = training_set.keys()
        x = numpy.matrix([[1, x, y] for x, y in k])
...
if __name__ == '__main__':
    ...
    for i in range(n_runs):
        ...
        out_samples = generate_samples(f, n_samples)
        lregression = Regression()
        lregression.train(samples)
        w = list(itertools.chain(*lregression.weights.tolist()))
        perceptron = perceptron.Perceptron(weights=w[1:], bias=w[0])
        n_iterations += perceptron.train(samples)
    ...
    print "p_correct C_out {0}".format(float(n_correct_out) / (n_samples * n_runs))
    print "p_false E_out {0}".format(float(n_false_out) / (n_samples * n_runs))
